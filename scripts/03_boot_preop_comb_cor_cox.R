...
app_start_cox <- coxph(Surv(BCR_FreeTime, BCR_Event) ~ 1, data = cor_df, x = TRUE)
app_full_cox <- coxph(Surv(BCR_FreeTime, BCR_Event) ~ ., data = cor_df, x = TRUE)
